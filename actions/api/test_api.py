...
if __name__ == '__main__':
    ...
    user_query = {'user_query': '校园卡丢了'}

    def test_cqa_api(idx):
        url = 'http://127.0.0.1:7070/cqa'
        timestamp = time.time()
        formatted_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp))
        print(f'线程{idx}开始时间：{formatted_time}')
        response = requests.post(url, json=user_query)
        end_timestamp = time.time()
        formatted_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_timestamp))
        print(f'线程{idx}结束时间：{formatted_time}')
        print(f'线程{idx}花费时间：{end_timestamp - timestamp}')
    ...
    def test_dqa_api(idx):
        url = 'http://127.0.0.1:9090/dqa'
        timestamp = time.time()
        formatted_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(timestamp))
        print(f'线程{idx}开始时间：{formatted_time}')
        response = requests.post(url, json=user_query)
    ...
    for i in range(concurrent_num):
        t = threading.Thread(target=test_dqa_api, kwargs={'idx': i})
        threads.append(t)
        t.start()
